...
**Note — AI-generated session title regeneration (P1):**
The initial title for a session is generated by `ConversationTitleService` after the first
message exchange via a one-off `IAgentService` call that does not touch the user's
conversation thread.  When soft consolidation fires in Epic 3 (prompt_tokens > 50% of
model context limit), the session title should be regenerated using the same mechanism so
that long-running conversations with topic shifts are reflected accurately in the session
list.  Hook the call into `ChatApplicationService` at the point where the consolidation
cursor is updated — check whether the snapshot title is stale (older than the new
consolidation point) and trigger a fresh `IConversationTitleService.GenerateTitleAsync`
call with the summarised context.
